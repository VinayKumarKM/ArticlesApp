...
def todos(request):
    if request.session.get('User') is not None:
        ...
        todos = Todo.objects.filter(user=user, is_deleted=False)
        todos_list = list()
        for todo in todos:
        ...
        return render(request, template, {'todos': todos_list, 'user': user})
    else:
        return redirect('home')
...
def comment(request, todo_id):
    if request.session.get('User') is not None and request.is_ajax():
        ...
        comment.save()
        single_comment = dict()
        single_comment['comment'] = comment.description
        single_comment['todo_name'] = todo.name
        return HttpResponse(json.dumps(single_comment))
